...
import { useApproveTransferFundMutation, useGetPendingTransferTransactionQuery } from "../services/api.ts";

const TransferApproval = () => {
    const { data, isLoading } = useGetPendingTransferTransactionQuery();
    const [approveTransfer] = useApproveTransferFundMutation();
    ...
    const handleUpdateStatus = async (transactionId: string) => {
        await approveTransfer(transactionId);
    ...
    return (
        <Box>
            {transactions.length === 0 ? (
                <Typography textAlign="center" color="#666">No pending transfers</Typography>
            ) : (
                        ...
                        <Grid item xs={12} sm={6} md={4} key={transaction._id}>
                            <motion.div whileHover={{ scale: 1.05 }}>
                                <Card sx={{ borderRadius: 3, boxShadow: "0px 4px 8px rgba(0,0,0,0.1)" }}>
                                    <CardContent>
                                        <Typography fontWeight={600} color="#444">TRANSFER</Typography>
